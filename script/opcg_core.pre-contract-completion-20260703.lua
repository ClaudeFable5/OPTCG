...
local function play_from_deck_top(action, context)
	...
	top:Sub(selected)
	if top:GetCount() > 0 then
		if action.rest_destination == "TRASH" then Duel.SendtoGrave(top, REASON_EFFECT)
...
local function look_reorder_deck_top(action, context)
	...
	if to_bottom and to_top then
		-- "top OR bottom": the chooser sends each looked-at card to either pile.
		-- Picking which ones go to the bottom (0..N) covers every split, so
		-- "all to the bottom" and "all to the top" are both reachable. Each pile
		-- then keeps a chooser-decided order.
		local bottom_g = top:Select(chooser, 0, count, nil)
		local top_g = top:Clone()
		top_g:Sub(bottom_g)
		if bottom_g:GetCount() > 0 then
			Duel.SendtoDeck(bottom_g, player, SEQ_DECKBOTTOM, REASON_EFFECT)
			if bottom_g:GetCount() > 1 then Duel.SortDeckbottom(chooser, player, bottom_g:GetCount()) end
		end
		if top_g:GetCount() > 1 then Duel.SortDecktop(chooser, player, top_g:GetCount()) end
...
local function action_shape_supported(action, card)
	...
	if action.op == "MODIFY_NEXT_PLAY_COST" then
		-- only the "this turn, next play from hand" shape is modelled
		if action.duration ~= nil and action.duration ~= "THIS_TURN" then return false end
...
local function effect_shape_supported(effect, card)
	...
	for _, action in ipairs(effect.actions or {}) do
		...
		if action.op == "CANNOT_BE_KO" then
			if not KO_REASON[action.reason] or action.limit or action.source_filter then return false end
			if action.attacker_filter then
				if action.reason ~= "BATTLE" and action.reason ~= "OPPONENT_BATTLE" then return false end
				if not filter_for(action.attacker_filter, { card=card }) then return false end
			end
		end
	end
	return true
...
local function register_event_play(card, effect)
	...
	native:SetOperation(function(e, tp)
		opcg.runtime.resolve(e:GetHandler(), effect.effect_id,
			runtime_context(e:GetHandler(), tp))
	end)
	card:RegisterEffect(native)
	return true
		...
		if effect_shape_supported(effect, card) then
			...
			for _, timing in ipairs(effect.timings or {}) do
				...
				if timing == "ON_PLAY" and (opcg.IsCharacter(card) or opcg.IsLeader(card)) then
					-- characters enter play via the NORMAL summon proc (play_proc), effects
					-- play via SpecialSummon -- ON_PLAY must listen to BOTH success events
					register_trigger(card, effect, EVENT_SUMMON_SUCCESS, LOCATION_MZONE, effect_index)
					register_trigger(card, effect, EVENT_SPSUMMON_SUCCESS, LOCATION_MZONE, effect_index)
				end
